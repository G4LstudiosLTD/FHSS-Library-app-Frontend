...
function UserManagementStudent() {
  ...
  ?.filter((student: any) => {
    if (filter === "junior") return student.grade.includes("JSS");
    if (filter === "senior") return student.grade.includes("SSS");
    return true;
  })
  .filter(
    (student: any) =>
    ...
    <div className="w-full h-screen bg-white">
      ...
      <div className="pt-6 w-full h-full">
        <div className="relative px-12 w-full">
          ...
          <div className="w-full flex items-center justify-between mt-8 flex-wrap gap-4 ">
            <div className="flex items-center border rounded-lg h-[40px] overflow-hidden">
              <p 
                className={`cursor-pointer flex items-center border-r h-full ${filter === 'all' && 'bg-[#e2e2e2]'}`}
                onClick={() => setFilter('all')}
              >
                <span className='px-3'>View all</span>
              </p>
              <p
                className={`cursor-pointer flex items-center px-3 border-r h-full ${filter === 'junior' && 'bg-[#e2e2e2]'}`}
                onClick={() => setFilter('junior')}
               >
                Junior
              </p>
              <p
                className={`cursor-pointer h-full flex items-center justify-center ${filter === 'senior' && 'bg-[#e2e2e2]'}`}
                onClick={() => setFilter('senior')}
              >
                <span className='px-3'>Senior</span>
              </p>
            </div>
